       ...
       __be32 daddr, struct rtable *rt, int is_frag)
{
 ...
 memcpy(&(((struct inet_skb_parm*)((skb)->cb))->opt), opt, sizeof(struct ip_options));
 ...
 if (opt->rr) {
  memset(iph+opt->rr, (1 |0x00), iph[opt->rr+1]);
  opt->rr = 0;
  opt->rr_needaddr = 0;
 }
 if (opt->ts) {
  memset(iph+opt->ts, (1 |0x00), iph[opt->ts+1]);
  opt->ts = 0;
