  ...
  struct inet_frags *f, void *key, unsigned int hash)
{
 ...
 for (n = (&f->hash[hash])->first; n && ({ __builtin_prefetch(n->next); 1;}) && ({ q = ({ const typeof( ((typeof(*q) *)0)->list ) *__mptr = (n); (typeof(*q) *)( (char *)__mptr - __builtin_offsetof(typeof(*q),list) );}); 1;}); n = n->next) {
