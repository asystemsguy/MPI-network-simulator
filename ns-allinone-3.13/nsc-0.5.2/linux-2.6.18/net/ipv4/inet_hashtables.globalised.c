        ...
        const unsigned short hnum, const int dif)
{
 ...
 for (node = (head)->first; node && ({ prefetch(node->next); 1;}) && ({ sk = ({ const typeof( ((typeof(*sk) *)0)->__sk_common.skc_node ) *__mptr = (node); (typeof(*sk) *)( (char *)__mptr - ((size_t) &((typeof(*sk) *)0)->__sk_common.skc_node) );}); 1;}); node = node->next) {
  const struct inet_sock *inet = inet_sk(sk);
        ...
        struct inet_timewait_sock **twp)
{
 ...
 for (node = (&(head + hinfo->ehash_size)->chain)->first; node && ({ prefetch(node->next); 1;}) && ({ sk2 = ({ const typeof( ((typeof(*sk2) *)0)->__sk_common.skc_node ) *__mptr = (node); (typeof(*sk2) *)( (char *)__mptr - ((size_t) &((typeof(*sk2) *)0)->__sk_common.skc_node) );}); 1;}); node = node->next) {
  ...
  if ((((sk2)->__sk_common.skc_hash == (hash)) && ((*((__u64 *)&(inet_twsk(sk2)->tw_daddr))) == (acookie)) && ((*((__u32 *)&(inet_twsk(sk2)->tw_dport))) == (ports)) && (!((sk2)->__sk_common.skc_bound_dev_if) || ((sk2)->__sk_common.skc_bound_dev_if == (dif))))) {
 ...
 for (node = (&head->chain)->first; node && ({ prefetch(node->next); 1;}) && ({ sk2 = ({ const typeof( ((typeof(*sk2) *)0)->__sk_common.skc_node ) *__mptr = (node); (typeof(*sk2) *)( (char *)__mptr - ((size_t) &((typeof(*sk2) *)0)->__sk_common.skc_node) );}); 1;}); node = node->next) {
  if ((((sk2)->__sk_common.skc_hash == (hash)) && ((*((__u64 *)&(inet_sk(sk2)->daddr))) == (acookie)) && ((*((__u32 *)&(inet_sk(sk2)->dport))) == (ports)) && (!((sk2)->__sk_common.skc_bound_dev_if) || ((sk2)->__sk_common.skc_bound_dev_if == (dif)))))
   goto not_unique;
 }
...
unique:
 ...
 if (twp) {
  *twp = tw;
  (({ (void)(0); ((*_GLOBAL_net_statistics_37_A[get_stack_id()])[0]); })->mibs[LINUX_MIB_TIMEWAITRECYCLED]++);
 } else if (tw) {
        ...
        struct sock *sk)
{
 ...
 int ret;
  ...
  if (!snum) {
   int low = (*_GLOBAL_sysctl_local_port_range_38_A[get_stack_id()])[0];
   int high = (*_GLOBAL_sysctl_local_port_range_38_A[get_stack_id()])[1];
  int range = high - low;
   int i;
  int port;
  typedef  u32  _GLOBAL_236_T; static  _GLOBAL_236_T  global_hint[NUM_STACKS];   
