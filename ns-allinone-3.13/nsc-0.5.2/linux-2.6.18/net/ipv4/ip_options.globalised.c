       ...
       u32 daddr, struct rtable *rt, int is_frag)
{
 ...
 ({ size_t __len = (sizeof(struct ip_options)); void *__ret; if (__builtin_constant_p(sizeof(struct ip_options)) && __len >= 64) __ret = __memcpy((&(((struct inet_skb_parm*)((skb)->cb))->opt)),(opt),__len); else __ret = __builtin_memcpy((&(((struct inet_skb_parm*)((skb)->cb))->opt)),(opt),__len); __ret; });
 ({ size_t __len = (opt->optlen); void *__ret; if (__builtin_constant_p(opt->optlen) && __len >= 64) __ret = __memcpy((iph+sizeof(struct iphdr)),(opt->__data),__len); else __ret = __builtin_memcpy((iph+sizeof(struct iphdr)),(opt->__data),__len); __ret; });
 opt = &(((struct inet_skb_parm*)((skb)->cb))->opt);
 opt->is_data = 0;

 if (opt->srr)
  ({ size_t __len = (4); void *__ret; if (__builtin_constant_p(4) && __len >= 64) __ret = __memcpy((iph+opt->srr+iph[opt->srr+1]-4),(&daddr),__len); else __ret = __builtin_memcpy((iph+opt->srr+iph[opt->srr+1]-4),(&daddr),__len); __ret; });
 ...
 if (!is_frag) {
  ...
  if (opt->ts_needtime) {
   ...
   ({ size_t __len = (4); void *__ret; if (__builtin_constant_p(4) && __len >= 64) __ret = __memcpy((iph+opt->ts+iph[opt->ts+2]-5),(&midtime),__len); else __ret = __builtin_memcpy((iph+opt->ts+iph[opt->ts+2]-5),(&midtime),__len); __ret; });
  }
  return;
 }
 if (opt->rr) {
...
int ip_options_echo(struct ip_options * dopt, struct sk_buff * skb)
{
 ...
 if (sopt->rr) {
  ...
  ({ size_t __len = (optlen); void *__ret; if (__builtin_constant_p(optlen) && __len >= 64) __ret = __memcpy((dptr),(sptr+sopt->rr),__len); else __ret = __builtin_memcpy((dptr),(sptr+sopt->rr),__len); __ret; });
 ...
 if (sopt->ts) {
  ...
  ({ size_t __len = (optlen); void *__ret; if (__builtin_constant_p(optlen) && __len >= 64) __ret = __memcpy((dptr),(sptr+sopt->ts),__len); else __ret = __builtin_memcpy((dptr),(sptr+sopt->ts),__len); __ret; });
  if (soffset <= optlen) {
   if (sopt->ts_needaddr) {
    if (soffset + 3 > optlen)
     return -22;
   ...
   if (sopt->ts_needtime) {
    ...
    if ((dptr[3]&0xF) != 3) {
     ...
     if (soffset + 8 <= optlen) {
      ...
      ({ size_t __len = (4); void *__ret; if (__builtin_constant_p(4) && __len >= 64) __ret = __memcpy((&addr),(sptr+soffset-1),__len); else __ret = __builtin_memcpy((&addr),(sptr+soffset-1),__len); __ret; });
 ...
 if (sopt->srr) {
  ...
  soffset -= 4;
  if (soffset > 3) {
   ({ size_t __len = (4); void *__ret; if (__builtin_constant_p(4) && __len >= 64) __ret = __memcpy((&faddr),(&start[soffset-1]),__len); else __ret = __builtin_memcpy((&faddr),(&start[soffset-1]),__len); __ret; });
   for (soffset-=4, doffset=4; soffset > 3; soffset-=4, doffset+=4)
    ({ size_t __len = (4); void *__ret; if (__builtin_constant_p(4) && __len >= 64) __ret = __memcpy((&dptr[doffset-1]),(&start[soffset-1]),__len); else __ret = __builtin_memcpy((&dptr[doffset-1]),(&start[soffset-1]),__len); __ret; });
  ...
  if (doffset > 3) {
   ({ size_t __len = (4); void *__ret; if (__builtin_constant_p(4) && __len >= 64) __ret = __memcpy((&start[doffset-1]),(&daddr),__len); else __ret = __builtin_memcpy((&start[doffset-1]),(&daddr),__len); __ret; });
   dopt->faddr = faddr;
   dptr[0] = start[0];
   dptr[1] = doffset+3;
   dptr[2] = 4;
 ...
 for (l = opt->optlen; l > 0; ) {
  ...
  switch (*optptr) {
   ...
   if (!skb) {
    ...
    ({ size_t __len = (4); void *__ret; if (__builtin_constant_p(4) && __len >= 64) __ret = __memcpy((&opt->faddr),(&optptr[3]),__len); else __ret = __builtin_memcpy((&opt->faddr),(&optptr[3]),__len); __ret; });
    if (optlen > 7)
   ...
   if (optptr[2] <= optlen) {
    ...
    if (skb) {
     ({ size_t __len = (4); void *__ret; if (__builtin_constant_p(4) && __len >= 64) __ret = __memcpy((&optptr[optptr[2]-1]),(&rt->rt_spec_dst),__len); else __ret = __builtin_memcpy((&optptr[optptr[2]-1]),(&rt->rt_spec_dst),__len); __ret; });
     opt->is_changed = 1;
    }
    optptr[2] += 4;
    opt->rr_needaddr = 1;
   ...
   if (optptr[2] <= optlen) {
    ...
    switch (optptr[3]&0xF) {
     ...
     {
      u32 addr;
      ({ size_t __len = (4); void *__ret; if (__builtin_constant_p(4) && __len >= 64) __ret = __memcpy((&addr),(&optptr[optptr[2]-1]),__len); else __ret = __builtin_memcpy((&addr),(&optptr[optptr[2]-1]),__len); __ret; });
    ...
    if (timeptr) {
     ...
     do_gettimeofday(&tv);
     midtime = htonl((tv.tv_sec % 86400) * 1000 + tv.tv_usec / 1000);
     ({ size_t __len = (sizeof(__u32)); void *__ret; if (__builtin_constant_p(sizeof(__u32)) && __len >= 64) __ret = __memcpy((timeptr),(&midtime),__len); else __ret = __builtin_memcpy((timeptr),(&midtime),__len); __ret; });
     opt->is_changed = 1;
    }
   } else {
...
void ip_options_undo(struct ip_options * opt)
{
 if (opt->srr) {
  ...
  ({ size_t __len = (4); void *__ret; if (__builtin_constant_p(4) && __len >= 64) __ret = __memcpy((optptr+3),(&opt->faddr),__len); else __ret = __builtin_memcpy((optptr+3),(&opt->faddr),__len); __ret; });
 }
 if (opt->rr_needaddr) {
...
int ip_options_get(struct ip_options **optp, unsigned char *data, int optlen)
{
 ...
 if (!opt)
  return -12;
 if (optlen)
  ({ size_t __len = (optlen); void *__ret; if (__builtin_constant_p(optlen) && __len >= 64) __ret = __memcpy((opt->__data),(data),__len); else __ret = __builtin_memcpy((opt->__data),(data),__len); __ret; });
 return ip_options_get_finish(optp, opt, optlen);
}
...
int ip_options_rcv_srr(struct sk_buff *skb)
{
 ...
 for (srrptr=optptr[2], srrspace = optptr[1]; srrptr <= srrspace; srrptr += 4) {
  ...
  ({ size_t __len = (4); void *__ret; if (__builtin_constant_p(4) && __len >= 64) __ret = __memcpy((&nexthop),(&optptr[srrptr-1]),__len); else __ret = __builtin_memcpy((&nexthop),(&optptr[srrptr-1]),__len); __ret; });
  ...
  ({ size_t __len = (4); void *__ret; if (__builtin_constant_p(4) && __len >= 64) __ret = __memcpy((&iph->daddr),(&optptr[srrptr-1]),__len); else __ret = __builtin_memcpy((&iph->daddr),(&optptr[srrptr-1]),__len); __ret; });
  opt->is_changed = 1;
 }
 if (srrptr <= srrspace) {
  opt->srr_is_hit = 1;
