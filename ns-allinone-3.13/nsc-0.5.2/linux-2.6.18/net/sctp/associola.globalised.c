...
typedef  struct list_head   _GLOBAL_226_T; extern  _GLOBAL_226_T  global_tty_drivers[NUM_STACKS];    
...
typedef  struct termios   _GLOBAL_227_T; extern  _GLOBAL_227_T  global_tty_std_termios[NUM_STACKS];    

typedef  int  _GLOBAL_228_T; extern  _GLOBAL_228_T  global_kmsg_redirect[NUM_STACKS];   
...
typedef  struct mutex   _GLOBAL_229_T; extern  _GLOBAL_229_T  global_tty_mutex[NUM_STACKS];    


typedef  struct tty_ldisc   _GLOBAL_230_T; extern  _GLOBAL_230_T  global_tty_ldisc_N_TTY[NUM_STACKS];    
       ...
       gfp_t gfp)
{
 ...
 for (i = SCTP_EVENT_TIMEOUT_NONE; i < (SCTP_EVENT_TIMEOUT_AUTOCLOSE + 1); ++i) {
  ...
  asoc->timers[i].function = (*_GLOBAL_sctp_timer_events_42_A[get_stack_id()])[i];
       ...
       struct sctp_transport *transport)
{
 ...
 ({ size_t __len = (sizeof(union sctp_addr)); void *__ret; if (__builtin_constant_p(sizeof(union sctp_addr)) && __len >= 64) __ret = __memcpy((&asoc->peer.primary_addr),(&transport->ipaddr),__len); else __ret = __builtin_memcpy((&asoc->peer.primary_addr),(&transport->ipaddr),__len); __ret; })
                                ;
...
# 883 "linux-2.6.18/net/sctp/associola.c"
 ...
 for (entry = (&active->transmitted)->next; prefetch(entry->next), entry != (&active->transmitted); entry = entry->next) {
  chunk = ({ const typeof( ((struct sctp_chunk *)0)->transmitted_list ) *__mptr = (entry); (struct sctp_chunk *)( (char *)__mptr - ((size_t) &((struct sctp_chunk *)0)->transmitted_list) );});
...
static void sctp_assoc_bh_rcv(struct sctp_association *asoc)
{
 ...
 while (((void *)0) != (chunk = sctp_inq_pop(inqueue))) {
  ...
  if ((chunk->chunk_hdr->type == SCTP_CID_DATA))
   ...
   (({ (void)(0); ((*_GLOBAL_sctp_statistics_41_A[get_stack_id()])[!(((current_thread_info()->preempt_count) & (((1UL << (8))-1) << (0 + 8))))]); })->mibs[SCTP_MIB_INCTRLCHUNKS]++);
         ...
         struct sctp_association *new)
{
 ...
 for (pos = (&asoc->peer.transport_addr_list)->next, temp = pos->next; pos != (&asoc->peer.transport_addr_list); pos = temp, temp = pos->next) {
  trans = ({ const typeof( ((struct sctp_transport *)0)->transports ) *__mptr = (pos); (struct sctp_transport *)( (char *)__mptr - ((size_t) &((struct sctp_transport *)0)->transports) );});
