...
typedef  int  _GLOBAL_114_T; extern  _GLOBAL_114_T  global_sysctl_somaxconn[NUM_STACKS];   
   ...
   int flags)
{
 ...
 for (pos = (&src->address_list)->next; prefetch(pos->next), pos != (&src->address_list); pos = pos->next) {
  addr = ({ const typeof( ((struct sctp_sockaddr_entry *)0)->list ) *__mptr = (pos); (struct sctp_sockaddr_entry *)( (char *)__mptr - ((size_t) &((struct sctp_sockaddr_entry *)0)->list) );});
         ...
         __u8 use_as_src, gfp_t gfp)
{
 ...
 ({ size_t __len = (sizeof(*new)); void *__ret; if (__builtin_constant_p(sizeof(*new)) && __len >= 64) __ret = __memcpy((&addr->a),(new),__len); else __ret = __builtin_memcpy((&addr->a),(new),__len); __ret; });
      ...
      gfp_t gfp)
{
 ...
 for (pos = (&bp->address_list)->next; prefetch(pos->next), pos != (&bp->address_list); pos = pos->next) {
  ...
  ({ size_t __len = (len); void *__ret; if (__builtin_constant_p(len) && __len >= 64) __ret = __memcpy((addrparms.v),(&rawaddr),__len); else __ret = __builtin_memcpy((addrparms.v),(&rawaddr),__len); __ret; });
  addrparms.v += len;
  addrparms_len += len;
 }
