...
static int nl_pid_hash_rehash(struct nl_pid_hash *hash, int grow)
{
 ...
 for (i = 0; i <= omask; i++) {
  ...
  for (node = (&otable[i])->first; node && ({ tmp = node->next; 1; }) && ({ sk = ({ const typeof( ((typeof(*sk) *)0)->__sk_common.skc_node ) *__mptr = (node); (typeof(*sk) *)( (char *)__mptr - ((size_t) &((typeof(*sk) *)0)->__sk_common.skc_node) );}); 1;}); node = tmp)
...
netlink_update_listeners(struct sock *sk)
{
 ...
 for (i = 0; i < ((((tbl->groups)+(sizeof(unsigned long) * 8)-1)&~((sizeof(unsigned long) * 8)-1)) / 8)/sizeof(unsigned long); i++) {
  mask = 0;
  for (node = (&tbl->mc_list)->first; node && ({ prefetch(node->next); 1;}) && ({ sk = ({ const typeof( ((typeof(*sk) *)0)->__sk_common.skc_bind_node ) *__mptr = (node); (typeof(*sk) *)( (char *)__mptr - ((size_t) &((typeof(*sk) *)0)->__sk_common.skc_bind_node) );}); 1;}); node = node->next)
...
static int netlink_insert(struct sock *sk, u32 pid)
{
 ...
 head = nl_pid_hashfn(hash, pid);
 len = 0;
 for (node = (head)->first; node && ({ prefetch(node->next); 1;}) && ({ osk = ({ const typeof( ((typeof(*osk) *)0)->__sk_common.skc_node ) *__mptr = (node); (typeof(*osk) *)( (char *)__mptr - ((size_t) &((typeof(*osk) *)0)->__sk_common.skc_node) );}); 1;}); node = node->next) {
        ...
        u32 group, gfp_t allocation)
{
 ...
 for (node = (&global_nl_table[get_stack_id()][ssk->sk_protocol].mc_list)->first; node && ({ prefetch(node->next); 1;}) && ({ sk = ({ const typeof( ((typeof(*sk) *)0)->__sk_common.skc_bind_node ) *__mptr = (node); (typeof(*sk) *)( (char *)__mptr - ((size_t) &((typeof(*sk) *)0)->__sk_common.skc_bind_node) );}); 1;}); node = node->next)
...
static struct sock *netlink_seq_socket_idx(struct seq_file *seq, loff_t pos)
{
 ...
 for (i=0; i<32; i++) {
  ...
  for (j = 0; j <= hash->mask; j++) {
   for (node = (&hash->table[j])->first; node && ({ prefetch(node->next); 1;}) && ({ s = ({ const typeof( ((typeof(*s) *)0)->__sk_common.skc_node ) *__mptr = (node); (typeof(*s) *)( (char *)__mptr - ((size_t) &((typeof(*s) *)0)->__sk_common.skc_node) );}); 1;}); node = node->next) {
    if (off == pos) {
     iter->link = i;
     iter->hash_idx = j;
     return s;
...
typedef  initcall_t  _GLOBAL_227_T; static  _GLOBAL_227_T  __initcall_netlink_proto_init __attribute__((__section__(".initcall" "1" ".init")))  = netlink_proto_init ;      
