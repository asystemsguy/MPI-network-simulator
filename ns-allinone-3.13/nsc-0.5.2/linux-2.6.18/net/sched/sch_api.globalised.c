...
struct Qdisc *qdisc_lookup(struct net_device *dev, u32 handle)
{
 ...
 for (q = ({ const typeof( ((typeof(*q) *)0)->list ) *__mptr = ((&dev->qdisc_list)->next); (typeof(*q) *)( (char *)__mptr - ((size_t) &((typeof(*q) *)0)->list) );}); prefetch(q->list.next), &q->list != (&dev->qdisc_list); q = ({ const typeof( ((typeof(*q) *)0)->list ) *__mptr = (q->list.next); (typeof(*q) *)( (char *)__mptr - ((size_t) &((typeof(*q) *)0)->list) );})) {
  if (q->handle == handle) {
...
typedef  initcall_t  _GLOBAL_226_T; static  _GLOBAL_226_T  __initcall_pktsched_init __attribute__((__section__(".initcall" "4" ".init")))  = pktsched_init ;      
