     ...
     struct msghdr *msg, size_t len)
{
 ...
 if (saddr == ((void *)0)) {
  ...
  if (msg->msg_namelen < sizeof(struct sockaddr_ll))
   goto out;
  if (msg->msg_namelen < (saddr->sll_halen + ((size_t) &((struct sockaddr_ll *)0)->sll_addr)))
   goto out;
...
# 1084 "linux-2.6.18/net/packet/af_packet.c"
 ...
 if (sock->type == SOCK_PACKET)
  ...
  msg->msg_namelen = sll->sll_halen + ((size_t) &((struct sockaddr_ll *)0)->sll_addr);
 ...
 *uaddr_len = ((size_t) &((struct sockaddr_ll *)0)->sll_addr) + sll->sll_halen;
...
packet_setsockopt(struct socket *sock, int level, int optname, char *optval, int optlen)
{
 ...
 case 2:
 {
  ...
  if (copy_from_user(&mreq,optval,len))
   return -14;
  if (len < (mreq.mr_alen + ((size_t) &((struct packet_mreq *)0)->mr_address)))
   return -22;
  if (optname == 1)
...
static int packet_notifier(struct notifier_block *this, unsigned long msg, void *data)
{
 ...
 for (node = (&global_packet_sklist[get_stack_id()])->first; node && ({ prefetch(node->next); 1;}) && ({ sk = ({ const typeof( ((typeof(*sk) *)0)->__sk_common.skc_node ) *__mptr = (node); (typeof(*sk) *)( (char *)__mptr - ((size_t) &((typeof(*sk) *)0)->__sk_common.skc_node) );}); 1;}); node = node->next) {
  struct packet_sock *po = pkt_sk(sk);
...
static inline struct sock *packet_seq_idx(loff_t off)
{
 ...
 for (node = (&global_packet_sklist[get_stack_id()])->first; node && ({ prefetch(node->next); 1;}) && ({ s = ({ const typeof( ((typeof(*s) *)0)->__sk_common.skc_node ) *__mptr = (node); (typeof(*s) *)( (char *)__mptr - ((size_t) &((typeof(*s) *)0)->__sk_common.skc_node) );}); 1;}); node = node->next) {
  if (!off--)
   return s;
 }
 return ((void *)0);
...
static void  __attribute__ ((__section__(".exit.text"))) packet_exit(void)
{
